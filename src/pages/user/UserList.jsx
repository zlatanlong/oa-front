...
const UserList = (props) => {
  ...
  const identityOptions = [
    { name: '学生', value: 0 }, { name: '老师', value: 1 }, { name: '院长', value: 2 }
  ]
  const columns = [
    ...userColomns,
    {
      ...
      render: (txt, record, index) => {
        return <div><Button type="primary" size="small" onClick={()=>{props.history.push(`/user/edit/${record.id}`)}}>修改</Button></div>
      }
    }
  ];
    ...
    <div>
          ...
          <Row gutter={24}>
            ...
            <Col span={8}>
              <Form.Item name="identity" label='身份'>
                <Radio.Group>
                  {identityOptions.map(option => { return (<Radio value={option.value} key={option.value}>{option.name}</Radio>) })}
                </Radio.Group>
              </Form.Item>
            </Col>
          </Row>
      ...
      <Card><Table
        pagination={{
          ...
          onChange: (page, pageSize) => { pageCurrent.current = page; getPageData(queryData); },
          onShowSizeChange: (current, size) => { pageSize.current = size; getPageData(queryData); }
        }}
        columns={columns} dataSource={pageData} rowKey={row => row.id} /></Card>
    </div>
  );
}
